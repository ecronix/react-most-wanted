          ...
          <ListItem
            ...
            onClick={
              auth.uid === task.userId
                ? () => {
                  this.handleUpdateTask(key, { ...task, completed: !task.completed })
                }
                : undefined
            }
          >
            ...
            <ListItemText
              primary={task.title}
              secondary={`${task.userName} ${task.created ? intl.formatRelative(new Date(task.created)) : undefined}`}
            />
          ...
          <BottomNavigation style={{ width: '100%', position: 'absolute', bottom: 0, right: 0, left: 0, zIndex: 50 }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 15 }}>
              <TextField
                ...
                type="Text"
              />
              <IconButton disabled={this.state.value === ''} onClick={this.handleAddTask}>
